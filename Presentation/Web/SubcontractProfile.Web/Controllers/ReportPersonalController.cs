...
namespace SubcontractProfile.Web.Controllers
{
    public class ReportPersonalController : Controller
    {
        ...
        private string Lang = "";
        private readonly IStringLocalizer<ReportPersonalController> _localizer;
        public ReportPersonalController(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, IStringLocalizer<ReportPersonalController> localizer)
        {
        ...
        public IActionResult SearchPersonal(string citizen_id, string full_name, string contact_phone, string createFrom, string createTo)
        {
            var Result = new List<SubcontractProfilePersonalModel>();
            ...
            string uriString = string.Format("{0}/{1}/{2}/{3}/{4}/{5}", strpathAPI + "Personal/selectPersonal"
                , HttpUtility.UrlEncode(citizen_id, Encoding.UTF8)
                , HttpUtility.UrlEncode(full_name, Encoding.UTF8)
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                {
                    //data
                    Result = JsonConvert.DeserializeObject<List<SubcontractProfilePersonalModel>>(result);
                }
            }
